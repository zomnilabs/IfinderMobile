import {types} from '../actions/buildings';
...
export default function buildings(state = INITIAL_STATE, action) {
    if (state.persistVersion !== INITIAL_STATE.persistVersion) {
        return buildings(INITIAL_STATE, action);
    }
...
export const getBuildings = state => getActiveItems(state.buildings.items);